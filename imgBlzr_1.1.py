...
def add_watermark(img, text):
    """Add bottom-right watermark with PIL."""
    draw = ImageDraw.Draw(img)
    try:
        font = ImageFont.truetype('arial.ttf', 48)  # Scale with size; adjust if needed
    except:
        font = ImageFont.load_default()
    
    # Bottom-right: Margin 50px
    w, h = img.size
...
def blaze_images(args):
    ...
    if args.watermark:
        print(f"Watermark: '{args.watermark_text}' (bottom-right stealth mode)")
    ...
    for batch_tensors, filenames in dataloader:
        ...
        for i, (out_tensor, fname) in enumerate(zip(output_batch.cpu(), valid_fnames)):  # .cpu() here
            ...
            if args.watermark:
                out_img = add_watermark(out_img, args.watermark_text)
